...
function listCom(x){
	...
	if($('#region').length && $('#comuna').length)
	{
		iRegiones = 0;
		valorRegion = $('#region').val();
		htmlComuna = '<option value="">SELECCIONE COMUNA</option>';
		jQuery.each(RegionesYcomunas.REGIONES, function () {
			if (RegionesYcomunas.REGIONES[iRegiones].NOMBREREGION == valorRegion)
			{
				iComunas = 0;
				jQuery.each(RegionesYcomunas.REGIONES[iRegiones].COMUNAS, function () {
					...
					if(RegionesYcomunas.REGIONES[iRegiones].COMUNAS[iComunas]==x)
					{ htmlComuna = htmlComuna + ' selected';}
					htmlComuna = htmlComuna + '>' + RegionesYcomunas.REGIONES[iRegiones].COMUNAS[iComunas] + '</option>';
					iComunas++;
				});
			}
			iRegiones++;
...
function listCom2(x){
	...
	if($('#b_region').length && $('#b_comuna').length)
	{
		iRegiones = 0;
		valorRegion = $('#b_region').val();
		htmlComuna = '<option value="">SELECCIONE COMUNA</option>';
		jQuery.each(RegionesYcomunas.REGIONES, function () {
			if (RegionesYcomunas.REGIONES[iRegiones].NOMBREREGION == valorRegion)
			{
